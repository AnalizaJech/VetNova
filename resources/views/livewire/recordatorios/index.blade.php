<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {{-- Próximas Vacunas --}}
        <x-card title="Recordatorios de Vacunación" subtitle="Refuerzos pendientes para hoy y mañana" shadow class="border border-base-200">
            @if($vacunas->isEmpty())
                <div class="text-center py-8 text-base-content/40">
            ...
            @else
                <div class="divide-y divide-base-200/50">
                    @foreach($vacunas as $vacuna)
                        <div class="py-4 flex justify-between items-center">
                            <div>
                                <p class="font-bold text-base-content">{{ $vacuna->mascota->cliente->nombre_completo ?? 'Sin Cliente' }}</p>
                                <p class="text-xs text-base-content/60">
                                    Mascota: {{ $vacuna->mascota->nombre }} &bull; Refuerzo: {{ \Carbon\Carbon::parse($vacuna->proxima_dosis)->format('d/m/Y') }}
                                </p>
                                <p class="text-xs text-info">{{ $vacuna->nombre_vacuna }}</p>
                            </div>
                            <div>
                                @if($vacuna->mascota->cliente && $vacuna->mascota->cliente->telefono)
                                    <x-button icon="o-chat-bubble-left-ellipsis" class="btn-sm btn-success text-white" 
                                        wire:click="enviarWhatsApp('{{ $vacuna->mascota->cliente->telefono }}', '{{ $vacuna->mascota->cliente->nombres }}', 'Vacunación')" 
                                        tooltip="Enviar WhatsApp" spinner />
                                @else
                                    <x-badge value="Sin número" class="badge-error badge-sm" />
                                @endif
